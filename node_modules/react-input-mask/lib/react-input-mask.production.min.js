...
function _inheritsLoose(e, t) {
  (e.prototype = Object.create(t.prototype)),
    _defaults2((e.prototype.constructor = e), t);
}
...
function getInputSelection(e) {
  ...
  if ("selectionStart" in e && "selectionEnd" in e)
    ...
    a.parentElement() === e &&
      ((t = -a.moveStart("character", -e.value.length)),
      (n = -a.moveEnd("character", -e.value.length)));
  }
...
function parseMask(e, t, n) {
  var a = "",
    ...
    o = [];
  if (
    (t === undefined && (t = defaultMaskChar),
    null == n && (n = defaultFormatChars),
    !e || "string" != typeof e)
  )
    return {
      maskChar: t,
  ...
  return (
    e.split("").forEach(function (e) {
      s =
        (!s && "\\" === e) ||
        (s || !n[e]
          ? (o.push(a.length), a.length === o.length - 1 && (i += e))
          : (r = a.length + 1),
        (a += e),
        !1);
    }),
...
function formatValue(e, t) {
  ...
  if (!n) {
    for (
      (t = insertString(e, "", t, 0)).length < i.length && (t = i);
      t.length < a.length && isPermanentCharacter(e, t.length);
...
function insertString(r, o, e, s) {
  var l = r.mask,
    ...
    h = isFilled(r, o);
  return (
    !u && s > o.length && (o += l.slice(o.length, s)),
    t.every(function (e) {
      for (; (i = e), isPermanentCharacter(r, (a = s)) && i !== l[a]; ) {
        if (
          ...
          (t = e),
          (n = s),
          u && isPermanentCharacter(r, n) && t === u)
        )
          return !0;
      ...
      var t, n, a, i;
      return (
        (!isAllowedCharacter(r, s, e) && e !== u) ||
        (s < o.length
          ? (o =
              u || h || s < c.length
                ? o.slice(0, s) + e + o.slice(s + 1)
                : ((o = o.slice(0, s) + e + o.slice(s)), formatValue(r, o)))
          : u || (o += e),
        ++s < l.length)
      );
    }),
...
function processChange(e, t, n, a, i) {
  ...
  if (
    (n.end > i.start
      ? (h = (c = getInsertStringLength(e, a, (u = l.slice(i.start, n.end)), f))
          ? i.length
          : 0)
      : l.length < a.length && (h = a.length - l.length),
    (l = a),
    h)
  ) {
    if (1 === h && !i.length)
      f =
        i.start === n.start
          ? getRightEditablePosition(e, n.start)
          : getLeftEditablePosition(e, n.start);
    l = clearRange(e, l, f, h);
  }
  return (
    (l = insertString(e, l, u, f)),
    (f += c) >= r.length
      ? (f = r.length)
      : f < o.length && !c
      ? (f = o.length)
      : f >= o.length && f < s && c && (f = getRightEditablePosition(e, f)),
    u || (u = null),
    {
...
var InputElement = (function (c) {
  function e(e) {
    ...
    (f.focused = !1),
      (f.mounted = !1),
      (f.previousSelection = null),
      (f.selectionDeferId = null),
      (f.saveSelectionLoopDeferId = null),
      (f.saveSelectionLoop = function () {
        (f.previousSelection = f.getSelection()),
          (f.saveSelectionLoopDeferId = defer(f.saveSelectionLoop));
      }),
      (f.runSaveSelectionLoop = function () {
        null === f.saveSelectionLoopDeferId && f.saveSelectionLoop();
      }),
      (f.stopSaveSelectionLoop = function () {
        null !== f.saveSelectionLoopDeferId &&
          (cancelDefer(f.saveSelectionLoopDeferId),
          (f.saveSelectionLoopDeferId = null),
          (f.previousSelection = null));
      }),
      (f.getInputDOMNode = function () {
        if (!f.mounted) return null;
        var e = reactDom.findDOMNode(
            _assertThisInitialized(_assertThisInitialized(f))
          ),
      ...
      (f.setSelection = function (e, t, n) {
        ...
        a &&
          i &&
          (n.deferred || setInputSelection(a, e, t),
          null !== f.selectionDeferId && cancelDefer(f.selectionDeferId),
          (f.selectionDeferId = defer(function () {
      ...
      (f.onChange = function (e) {
        var t = _assertThisInitialized(
            _assertThisInitialized(f)
          ).beforePasteState,
          n = _assertThisInitialized(
            _assertThisInitialized(f)
          ).previousSelection,
          ...
          r = f.value,
          o = f.getSelection();
        f.isInputAutofilled(i, o, r, n) &&
          ((r = formatValue(f.maskOptions, "")),
          ...
          t &&
            ...
            (i = r.slice(0, n.start) + i + r.slice(n.end)),
            (f.beforePasteState = null));
        ...
        if (isFunction(a)) {
          var h = a(
            ...
            { value: r, selection: n },
            l,
            f.getBeforeMaskedValueChangeConfig()
          );
          (c = h.value), (u = h.selection);
        }
        f.setInputValue(c),
          isFunction(f.props.onChange) && f.props.onChange(e),
          f.isWindowsPhoneBrowser
            ? f.setSelection(u.start, u.end, { deferred: !0 })
      ...
      (f.onFocus = function (e) {
        ...
        if (((f.focused = !0), (f.mounted = !0), a)) {
          if (f.value)
            getFilledLength(f.maskOptions, f.value) <
              f.maskOptions.mask.length && f.setCursorToEnd();
          else {
            ...
            h && f.setInputValue(o),
              h && isFunction(f.props.onChange) && f.props.onChange(e),
              f.setSelection(u.start, u.end);
          }
      ...
      (f.onBlur = function (e) {
        ...
        if (
          (f.stopSaveSelectionLoop(),
          (f.focused = !1),
          n && !f.props.alwaysShowMask && isEmpty(f.maskOptions, f.value))
        ) {
          var a = "";
          if (isFunction(t))
            a = t(
              { value: a, selection: null },
              { value: f.value, selection: f.previousSelection },
              null,
              f.getBeforeMaskedValueChangeConfig()
            ).value;
          var i = a !== f.getInputValue();
          i && f.setInputValue(a),
            i && isFunction(f.props.onChange) && f.props.onChange(e);
        }
        isFunction(f.props.onBlur) && f.props.onBlur(e);
      }),
      (f.onMouseDown = function (e) {
        if (!f.focused && document.addEventListener) {
          (f.mouseDownX = e.clientX),
            (f.mouseDownY = e.clientY),
            (f.mouseDownTime = new Date().getTime());
          var r = function r(e) {
      ...
      (f.onPaste = function (e) {
        isFunction(f.props.onPaste) && f.props.onPaste(e),
          e.defaultPrevented ||
            ((f.beforePasteState = {
      ...
      (f.handleRef = function (e) {
        null == f.props.children &&
          isFunction(f.props.inputRef) &&
          f.props.inputRef(e);
      });
    var t = e.mask,
      n = e.maskChar,
    ...
    if (
      f.maskOptions.mask &&
      (i || l) &&
      ((l = formatValue(f.maskOptions, l)), isFunction(r))
    ) {
      var u = e.value;
      null == e.value && (u = o),
        (l = r(
          { value: l, selection: null },
          { value: (u = getStringValue(u)), selection: null },
          null,
          f.getBeforeMaskedValueChangeConfig()
        ).value);
    }
  ...
  return (
    ...
    (t.componentDidUpdate = function () {
      ...
      if (((this.maskOptions = parseMask(i, r, o)), this.maskOptions.mask)) {
        ...
        if (
          (s.mask || u || (c = this.getInputValue()),
          (f || (this.maskOptions.mask && (c || l))) &&
            (c = formatValue(this.maskOptions, c)),
          f)
        ) {
          var p = getFilledLength(this.maskOptions, c);
          (null === h || p < h) &&
            (h = isFilled(this.maskOptions, c)
              ? p
              : getRightEditablePosition(this.maskOptions, p));
        }
        !this.maskOptions.mask ||
          !isEmpty(this.maskOptions, c) ||
          l ||
          (u && this.props.value) ||
          (c = "");
        var d = { start: h, end: h };
        ...
        null != d.start &&
          null != d.end &&
          (v = !e || e.start !== d.start || e.end !== d.end),
          (v || g) && this.setSelection(d.start, d.end);
      } else s.mask && (this.stopSaveSelectionLoop(), this.forceUpdate());
    }),
    (t.componentWillUnmount = function () {
      (this.mounted = !1),
        null !== this.selectionDeferId && cancelDefer(this.selectionDeferId),
    ...
    (t.render = function () {
      ...
      if (n) {
        ...
        t = React.createElement("input", _extends({ ref: this.handleRef }, a));
      ...
      return (
        this.maskOptions.mask &&
          (a.disabled ||
            ...
            (o.onPaste = this.onPaste),
            (o.onMouseDown = this.onMouseDown)),
          null != a.value && (o.value = this.value)),
        (t = React.cloneElement(t, o))
      );
    }),
    e
  );
